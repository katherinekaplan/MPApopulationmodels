...
#' @param Lfish: length at first capture in fishery
...
#' @param MPAtime: the timestep that the MPA is implemented
#' @keywords open population, population dynamics
#' @examples openpop_plot(tf=50,M=0.2,Fi=0.14,Lfish=25,Linf=37.8,k=0.13,a0=-0.7,
#'maxage=25,pW=9.37e-06,qW=3.172,sig_r=0.3, R=500, MPAtime=1,simulations=100)


openpop_plot = function(tf,maxage,M,Fi,Lfish, Linf,k,a0,pW,qW,R,sig_r,simulations,MPAtime) {
  ...
  ##basic abundance an biomass ratio plot deterministic model
  plot1=ggplot(newdf, aes(x=time, y=Nratio))+
  ...
  ##
  plot2=ggplot(newdf, aes(x=time, y=Nrat.sim.mean))+
    geom_line(aes(y=Nrat.sim.mean,colour = "MPA" ),size=1)+
    geom_ribbon(data=newdf, aes(x=time,ymin=Nrat.lowerCI.MPA,
                                ymax=Nrat.upperCI.MPA), inherit.aes = FALSE,
                fill="lightblue", alpha=0.6)+
    geom_line(aes(y= Nrat.mean.noMPA,colour = "Fished"),size=1)+
    geom_ribbon(data=newdf, aes(x=time,ymin=Nrat.lowerCI.noMPA,
                                ymax=Nrat.upperCI.noMPA),
                fill="red",inherit.aes = FALSE, alpha=0.3)+
    ylab("Abundance ratio (Nt/N0)")+
    ylim(low=min(newdf$Nrat.lowerCI.noMPA),
         high=max(newdf$Nrat.upperCI.MPA))+
    xlab("time")+
    theme_gray()+
    scale_colour_discrete("")
  plot3= ggplot(newdf, aes(x=time, y=Bratio.sim.mean))+
    geom_line(aes(y=Bratio.sim.mean,colour = "MPA" ),size=1)+
    geom_ribbon(data=newdf, aes(x=time,ymin=Brat.lowerCI.MPA,
                                ymax=Brat.upperCI.MPA), inherit.aes = FALSE,
                fill="lightblue", alpha=0.6)+
    geom_line(aes(y= Brat.mean.noMPA,colour = "Fished"),size=1)+
    geom_ribbon(data=newdf, aes(x=time,ymin=Brat.lowerCI.noMPA,
                                ymax=Brat.upperCI.noMPA),
                fill="red",inherit.aes = FALSE, alpha=0.3)+
    ylab("Biomass ratio (Bt/B0)")+
    ylim(low=min(newdf$Brat.lowerCI.noMPA),
         high=max(newdf$Brat.upperCI.MPA))+
    xlab("time")+
    theme_gray()+
    scale_colour_discrete("")
  out=plot_grid(plot1,plot2,plot3, ncol = 1)
  return(out)
}
